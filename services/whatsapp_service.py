...
class WhatsAppService:
    ...
    def download_media(self, media_url: str, media_info: dict = None) -> Optional[bytes]:
        ...
        try:
            ...
            if media_info:
                ...
                for url in alternative_urls:
                    ...
                    if content:
                        if self.environment == 'development':
                            self.logger.info("Successfully downloaded media using alternative URL")
                        return content
        ...
        try:
            ...
            for headers in header_variants:
                ...
                if response.status_code == 200:
                    ...
                    if content_size > 0:
                        if self.environment == 'development':
                            self.logger.info(f"Downloaded {content_size} bytes from {url}")
    ...
    def _generate_alternative_urls(self, media_info: dict) -> list:
        ...
        if original_url and self.session_name:
            if '/files/' in original_url and f'/{self.session_name}/' not in original_url:
                fixed_url = original_url.replace('/files/', f'/files/{self.session_name}/')
