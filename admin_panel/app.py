...
def users():
    ...
    try:
        conn = get_db_connection()
        users_data = conn.execute("SELECT * FROM users ORDER BY last_active DESC").fetchall()
        conn.close()
    ...
    if request.method == 'POST':
        try:
            ...
            conn.execute(
                ...
                (phone_number, role, points, datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            )
            conn.commit()
            conn.close()
...
def locations():
    ...
    try:
        conn = get_db_connection()
        locations_data = conn.execute("SELECT * FROM collection_points ORDER BY name").fetchall()
        conn.close()
...
def edit_location(location_id):
    ...
    try:
        ...
        location = conn.execute("SELECT * FROM collection_points WHERE id = ?", (location_id,)).fetchone()
        conn.close()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            conn.execute(
                ...
                (location_name, address, schedule_day, schedule_time, waste_types, contact, is_active, schedule_id)
            )
            conn.commit()
            conn.close()
        ...
        schedule = conn.execute("SELECT * FROM collection_schedules WHERE id = ?", (schedule_id,)).fetchone()
        conn.close()
        
        if schedule:
            return render_template('schedule_form.html', schedule=schedule, action='edit')
        else:
