...
class CollectionPointModel:
    ...
    def create_collection_point(self, data: Dict[str, Any], admin_phone: str = None) -> bool:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    data.get('id', f"cp_{datetime.now().strftime('%Y%m%d_%H%M%S')}"),
                    data['name'],
                    data['type'],
                    data['latitude'],
                    data['longitude'],
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    # Parse JSON fields
                    try:
                        point['accepted_waste_types'] = json.loads(point['accepted_waste_types'])
                    except:
    ...
    def update_collection_point(self, point_id: str, data: Dict[str, Any], admin_phone: str = None) -> bool:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                for key, value in data.items():
                    if key in ['name', 'type', 'latitude', 'longitude', 'schedule', 'contact', 'description']:
    ...
    def get_collection_points_by_waste_type(self, waste_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for point in points:
                if waste_type.lower() in [wt.lower() for wt in point['accepted_waste_types']]:
...
class CollectionScheduleModel:
    ...
    def update_schedule(self, schedule_id: int, data: Dict[str, Any], admin_phone: str = None) -> bool:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                for key, value in data.items():
                    if key in ['location_name', 'address', 'schedule_day', 'schedule_time', 'contact']:
                        update_fields.append(f"{key} = ?")
                        params.append(value)
