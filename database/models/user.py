...
class UserModel:
    ...
    def get_user_role(self, phone_number: str) -> str:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                result = cursor.fetchone()
                return result['role'] if result else self._determine_initial_role(phone_number)
                
        except Exception as e:
            logger.error(f"Error getting user role: {str(e)}")
            return self._determine_initial_role(phone_number)
    
    def update_user_role(self, phone_number: str, role: str, admin_phone: str = None) -> bool:
    ...
    def get_users_by_role(self, role: str, admin_phone: str = None) -> List[Dict[str, Any]]:
