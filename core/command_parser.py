...
class CommandParser:
    ...
    def parse_command(self, message: str, phone_number: str = None) -> Dict[str, Any]:
        ...
        command_text = (
            message_lower[1:] if message_lower.startswith("/") else message_lower
        )
        ...
        for cmd_key, cmd_info in COMMAND_MAPPING.items():
            if command in cmd_info["aliases"]:
                ...
                user_role = (
                    self._get_user_role(phone_number) if phone_number else "warga"
                )
        ...
        return {"status": "unknown_command", "command": command, "args": args}
