...
def _load_valid_keys():
    raw = os.getenv('ADMIN_API_KEYS') or os.getenv('API_KEYS') or os.getenv('ADMIN_API_KEY')
    if not raw:
        return set()
...
def require_api_key(view_func):
    ...
    def wrapped(*args, **kwargs):
        ...
        # Try common headers
        auth = request.headers.get('X-API-Key') or request.headers.get('X-API-KEY') or request.headers.get('Authorization')
        key = None
        if auth:
            auth = auth.strip()
