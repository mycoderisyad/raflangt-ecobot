...
def get_collection_points():
    ...
    try:
        ...
        return jsonify({"status": "success", "data": collection_points})
    except Exception as e:
        logger.error(f"Error getting collection points: {str(e)}")
        return (
            jsonify({"status": "error", "message": "Failed to get collection points"}),
            500,
        )
...
def get_collection_point(point_id):
    ...
    try:
        ...
        if point:
            return jsonify({"status": "success", "data": point})
        else:
            return (
                jsonify({"status": "error", "message": "Collection point not found"}),
                404,
            )
    except Exception as e:
        logger.error(f"Error getting collection point: {str(e)}")
        return (
            jsonify({"status": "error", "message": "Failed to get collection point"}),
            500,
        )
